...
function tempered_particle_filter{S<:AbstractFloat}(data::Vector{<:SVector}, Φ::Function, Ψ::Function,
                                                    F_ϵ, F_u, s_init::Vector{<:SVector};
                                                    verbose::Symbol = :high, fixed_sched::Vector{S} = zeros(0),
                                                    r_star::S = 2., c::S = 0.3, accept_rate::S = 0.4, target::S = 0.4,
                                                    tol::Float64 = 1e-3, resampling_method = :multinomial,
                                                    N_MH::Int = 1, n_particles::Int = 1000,
    ...
    for t = 1:T
        ...
        # Remove rows/columns of series with NaN values
        ## 2018-04-26, Andreas Noack: Temporarily disable handling of missing values
        nonmissing          = isfinite.(y_t)
        # y_t                 = y_t[nonmissing]
        # n_obs_t             = length(y_t)
        # Ψ_t                 = (x, ϵ) -> Ψ(x, ϵ)[nonmissing]
        # HH_t                = cov(F_u)[nonmissing, nonmissing]
        # inv_HH_t            = inv(HH_t)
        # det_HH_t            = det(HH_t)
        n_obs_t             = length(y_t)
        Ψ_t                 = Ψ
        ...
        #####################################
        # Draw random shock ϵ
        ϵ = [rand(F_ϵ) for i in 1:n_particles]

        # Forecast forward one time step
        s_t_nontempered = Φ.(s_lag_tempered, ϵ)

        # Error for each particle
        p_error = [y_t] .- Ψ.(s_t_nontempered, [SVector{n_obs_t}(zeros(n_obs_t))])
        ...
        while φ_old < 1
            ...
            s_t_nontempered, ϵ, accept_rate = mutation(Φ, Ψ_t, cov(F_ϵ), det_HH_t, inv_HH_t, φ_new, y_t,
                                                       s_t_nontempered, s_lag_tempered, ϵ, c, N_MH;
                                                       parallel = parallel)
            # if VERBOSITY[verbose] >= VERBOSITY[:high]
                # toc()
