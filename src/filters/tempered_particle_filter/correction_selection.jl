...
# end
function selection(normalized_weights::Vector{Float64}, s_lag_tempered::Vector{<:SVector},
                   s_t_nontempered::Vector{<:SVector}, ϵ::Vector{<:SVector};
                   resampling_method::Symbol = :multinomial)
    # Resampling
    id = resample(normalized_weights, method = resampling_method)

    # Update arrays for resampled indices
    s_lag_tempered_id  = s_lag_tempered[id]
    s_t_nontempered_id = s_t_nontempered[id]
    ϵ_id               = ϵ[id]

    return s_lag_tempered_id, s_t_nontempered_id, ϵ_id
end
